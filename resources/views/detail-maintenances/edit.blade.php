<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @endif
            <form method="POST" action="{{ route('detailmaintenance.update', $detail_maintenance->id) }}"
                class="max-w-sm bg-white mx-auto md:mx-0 p-10 rounded-2xl">
                @csrf
                @method('PATCH')
                <input type="hidden" name="id_maintenance" value="{{ $detail_maintenance->id_maintenance }}">
                <div class="mb-5">
                    <label for="detail" class="block mb-2 text-sm font-medium text-gray-900">Detail
                        Maintenance</label>
                    <input type="text" id="detail" name="detail" value="{{ $detail_maintenance->detail }}"
                ...
                <div class="mb-5">
                    <div class="relative">
                        ...
                        <input type="number" name="cost" id="cost" value="{{ $detail_maintenance->cost }}" aria-describedby="helper-text-explanation"
